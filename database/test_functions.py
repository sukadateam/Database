...
backup.create(random_name=True, hide=False, password=encry_password)
backup.clear_all()
print('--(BACKUP TEST COMPLETE)--')
check_settingsImproved(hide=False)
...
profanityFilter.filter('Bitch', hide=False, test=True)
print('--(PROFANITY TEST COMPLETE)--')
encrypt_check()
print('\n--(HISTORY TEST)--')
history.clear()
history.create_history('He said Hello!', 'admin', manual_record=False, add_desc=True, desc='So I said hello back to him!', hide=False)
history.get_description(code='00000001', hide=False)
optimize.run(hide=False)
check_data(hide=False)
...
print('\n--(USER CLASS TEST)--')
users.create(new_user='test_dummy', new_password='testpas1',new_permission='admin', hide=False)
users.disable(user='test_dummy', hide=False)
users.enable(user='test_dummy', hide=False)
users.change_permissions(user='test_dummy', new_permission='student', hide=False)
users.change_password(user='test_dummy', new_password='test_pas2', hide=False)
users.change_name(user='test_dummy', new_name='test_dummy1', hide=False)
users.remove(user='test_dummy1', hide=False)
print('--(USER CLASS TEST COMPLETE)--')
print('\n--(DATABASE CLASS TEST)--')
if check.data_base_exists(data_base='money')==False:
    data_base.create.database(data_base='money', type='list', hide=False)
...
password_restrictions.set_min_length(value=15)
password_restrictions.set_max_length(value=25)
